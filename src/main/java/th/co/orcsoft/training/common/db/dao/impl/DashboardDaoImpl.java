...
public class DashboardDaoImpl extends AbsCorDao implements DashboardDao {
	...
	public List<ProvinceModel> getProvinces() {
		...
		List<ProvinceModel> provinces = namedParameterJdbcTemplate.query(sql,
				new BeanPropertyRowMapper<ProvinceModel>(ProvinceModel.class));
		return provinces;
	}
	...
	public List<RegionModel> getRegions() {
		...
		List<RegionModel> regions = namedParameterJdbcTemplate.query(sql,
				new BeanPropertyRowMapper<RegionModel>(RegionModel.class));
		return regions;
	}
	...
	public List<GetElectionPartyDistrictsModel> getElectionPartyDistricts(int provinceId) {
		...
		List<GetElectionPartyDistrictsModel> ElectionPartyDistricts = namedParameterJdbcTemplate.query(sql,
				new BeanPropertyRowMapper<GetElectionPartyDistrictsModel>(GetElectionPartyDistrictsModel.class));
	...
	public List<GetElectionPartyRegionModel> getElectionPartyRegion(int regionId) {
		...
		List<GetElectionPartyRegionModel> ElectionPartyRegion = namedParameterJdbcTemplate.query(sql,
				new BeanPropertyRowMapper<GetElectionPartyRegionModel>(GetElectionPartyRegionModel.class));
	...
	public ArrayList<Integer> getNotApprovedDistrictsByProvince(int provinceId) {
		String sql = "select Province.PrvID as provinceId, Province.numDist as numberAllDistricts, Vote.DistNum as voteDistrictNumber "
				+ "from Province " + "left join Vote on Province.PrvID = Vote.PrvID " + "where Province.PrvID = "
				+ provinceId;

		List<ProvinceDistrictVote> districtProvinceList = namedParameterJdbcTemplate.query(sql,
				new BeanPropertyRowMapper<ProvinceDistrictVote>(ProvinceDistrictVote.class));
	...
	public List<GetSummaryElectionPartyDistrictsModel> getSummaryElectionPartyDistricts(int provinceId) {
		...
		List<GetSummaryElectionPartyDistrictsModel> SummaryElectionPartyDistricts = namedParameterJdbcTemplate
				.query(sql, new BeanPropertyRowMapper<GetSummaryElectionPartyDistrictsModel>(
						GetSummaryElectionPartyDistrictsModel.class));
